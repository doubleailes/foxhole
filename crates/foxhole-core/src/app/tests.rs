...
fn selecting_marks_conversation_read() {
    ...
    assert_eq!(app.convs.items[1].unread, 1);

    app.convs.focus = Pane::PeerList;
    app.handle_key(press(KeyCode::Down)); // select bob
    assert_eq!(app.convs.selected, 1);
    assert_eq!(app.convs.items[1].unread, 0, "viewing clears unread");
}
...
fn drafts_are_per_conversation() {
    ...
    assert_eq!(app.convs.items[0].draft, "to-alice");

    // Switch to bob via the peer list; bob's draft is independent/empty.
    app.convs.focus = Pane::PeerList;
    app.handle_key(press(KeyCode::Down));
    assert_eq!(app.convs.items[1].draft, "");

    app.convs.focus = Pane::Transmit;
    type_str(&mut app, "to-bob");
    assert_eq!(app.convs.items[1].draft, "to-bob");
    assert_eq!(
        app.convs.items[0].draft, "to-alice",
...
fn typing_is_ignored_outside_conversations() {
    ...
    app.handle_key(press(KeyCode::Tab));
    assert!(
        app.convs.items[0].draft.is_empty(),
...
fn transmit_targets_selected_peer() {
    ...
    app.convs.items[0].draft = "stray".to_string();
    app.convs.selected = 1;
    app.convs.items[1].draft = "  hello bob  ".to_string();
    ...
    assert_eq!(
        app.convs.items[1].messages.last().unwrap().text,
        "[TX] hello bob"
    );
    assert!(app.convs.items[1].draft.is_empty(), "sent draft cleared");
    assert_eq!(app.convs.items[0].draft, "stray", "other drafts untouched");
}

#[test]
fn transmit_ignores_blank_draft() {
    let mut app = App::new();
    app.convs.items[0].draft = "   ".to_string();
    app.handle_key(ctrl('s'));
...
fn purge_clears_selected_draft_only() {
    let mut app = App::new();
    app.convs.items[0].draft = "secret".to_string();
    app.convs.items[1].draft = "keep".to_string();
    app.handle_key(ctrl('x'));
    assert!(app.convs.items[0].draft.is_empty());
    assert_eq!(app.convs.items[1].draft, "keep");
    assert!(app.outbox.outbound.is_empty());
}
...
fn transmit_sends_title_and_resets_field() {
    let mut app = App::new();
    app.convs.items[0].draft_title = "  report  ".to_string();
    app.convs.items[0].draft = "all clear".to_string();
    ...
    assert_eq!(
        app.convs.items[0].messages.last().unwrap().text,
        "[TX] report: all clear",
        "title echoed ahead of the body"
    );
    assert!(app.convs.items[0].draft_title.is_empty(), "title cleared");
    assert_eq!(
...
fn purge_clears_title_too() {
    let mut app = App::new();
    app.convs.items[0].draft = "body".to_string();
    app.convs.items[0].draft_title = "title".to_string();
    app.convs.transmit_field = TransmitField::Title;
    app.handle_key(ctrl('x'));
    assert!(app.convs.items[0].draft.is_empty());
    assert!(app.convs.items[0].draft_title.is_empty());
    assert_eq!(app.convs.transmit_field, TransmitField::Body);
}
...
fn deliver_routes_to_peer_and_increments_unread() {
    ...
    let dave = app.convs.items.iter().find(|c| c.peer == "dave").unwrap();
    ...
    assert_eq!(app.convs.items[0].peer, "alice");
    assert_eq!(app.convs.items[0].unread, 0, "selected peer stays read");
}
...
fn p_queues_path_probe_for_focused_selection() {
    ...
    app.net.column = NetColumn::Peers;
    app.convs.selected = 0;
    let peer = app.convs.items[0].peer.clone();
...
fn upsert_peer_stamps_last_seen() {
    ...
    let conv = app.convs.items.iter().find(|c| c.peer == peer).unwrap();
...
fn start_conversation_validates_and_normalizes() {
    let mut app = App::new();
    let before = app.convs.items.len();
    ...
    assert_eq!(app.convs.items.len(), before + 1);
    let conv = app.convs.items.last().unwrap();
    assert_eq!(conv.peer, "a1b2c3d4e5f600112233445566778899");
    assert_eq!(conv.display_name.as_deref(), Some("Bravo-6"));
    assert!(conv.pinned);
    assert_eq!(app.convs.selected, app.convs.items.len() - 1);
    assert_eq!(app.active, Tool::Conversations);
    assert_eq!(app.convs.focus, Pane::Transmit);
...
fn start_conversation_reuses_existing_and_updates_alias() {
    ...
    let n = app.convs.items.len();
    assert!(app.start_conversation(&addr, "Renamed"));
    assert_eq!(app.convs.items.len(), n, "no duplicate thread");
    let conv = app.convs.items.iter().find(|c| c.peer == addr).unwrap();
...
fn transmit_stamps_id_and_sending_status() {
    let mut app = App::new();
    app.convs.items[0].draft = "hi".to_string();
    app.handle_key(ctrl('s'));
    let entry = app.convs.items[0].messages.last().unwrap();
    assert!(entry.id > 0);
...
fn set_msg_status_updates_matching_entry_and_marks_dirty() {
    let mut app = App::new();
    app.convs.items[0].draft = "yo".to_string();
    app.handle_key(ctrl('s'));
    let id = app.convs.items[0].messages.last().unwrap().id;
    app.outbox.dirty.clear();

    app.set_msg_status(id, MsgStatus::Delivered);
    assert_eq!(
        app.convs.items[0].messages.last().unwrap().status,
        MsgStatus::Delivered
    );
    let peer = app.convs.items[0].peer.clone();
...
fn fail_dropped_marks_entry_failed_and_errors() {
    let mut app = App::new();
    app.convs.items[0].draft = "sitrep".to_string();
...
fn t_key_cycles_trust_and_marks_dirty() {
    ...
    app.outbox.dirty.clear();
    assert_eq!(app.convs.items[0].trust, Trust::Unknown);

    app.handle_key(press(KeyCode::Char('t')));
    assert_eq!(app.convs.items[0].trust, Trust::Trusted);
    let peer = app.convs.items[0].peer.clone();
    assert!(
...
fn t_key_in_transmit_pane_types_not_cycles() {
    ...
    app.handle_key(press(KeyCode::Char('t')));
    assert_eq!(app.convs.items[0].draft, "t");
    assert_eq!(app.convs.items[0].trust, Trust::Unknown);
}
