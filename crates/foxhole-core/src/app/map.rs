...
/// The World Map tool's own state: the viewport, what is selected in it, the
/// layers drawn over it, and its one modal.
///
/// Grouped so the map's fields move together and a reader of this module can
/// see, from `self.map.…`, exactly which state the tool owns. The *intel*
/// layer is deliberately not in here — see [`App::intel`].
pub struct MapState {
    /// Viewport (pan/zoom state).
    pub view: MapView,
    /// Selected marker index within [`App::map_markers`].
    pub selected: usize,
    /// Whether the embedded capitals/cities reference layer is drawn (toggled
    /// with `g`). On by default for orientation.
    pub cities: bool,
    ...
    /// is *local* (operator-authored) intel — distinct from [`App::intel`].
    pub zones: Vec<Zone>,
...
mod tests {
    ...
    fn markers_list_operator_first_then_located_peers() {
        ...
        app.convs.list[0].location = Some(GeoPos::new(51.5, -0.1));
    ...
    fn telemetry_creates_a_peer_marker_and_refreshes_it() {
        ...
        assert_eq!(app.convs.list.len(), 1);
        let markers = app.map_markers();
    ...
    fn empty_inbound_message_is_not_recorded() {
        ...
        app.deliver("peer", "");
        assert!(app.convs.list.is_empty());
        // A real message is still delivered normally.
        app.deliver("peer", "hi");
        assert_eq!(app.convs.list.len(), 1);
        assert_eq!(app.convs.list[0].messages.len(), 1);
    }
    ...
    fn cycling_markers_wraps_and_recenters() {
        ...
        app.convs.list[0].location = Some(GeoPos::new(10.0, 20.0));
    ...
    fn centering_from_the_globe_zooms_in_to_frame() {
        let mut app = App::new();
        app.config = Config::default();
        app.convs.list[0].location = Some(GeoPos::new(40.0, 30.0));
