...
//! inbound delivery. All operate on [`ConversationsState`].

use super::*;
use crate::domain::{normalize_address, now_secs, short_hash};

/// Conversations tool state: the per-peer roster (which doubles as the Network
/// tab's Peers column) and the tool's focus/compose/scroll state.
pub struct ConversationsState {
    /// All conversations, in display order.
    pub items: Vec<Conversation>,
    /// Index of the selected conversation within `items`.
    pub selected: usize,
    /// Focused pane within the tool (drives the reversed highlight + key
    /// routing there). Ignored by the read-only tools.
    pub focus: Pane,
    ...
    /// Scroll position of the selected conversation's thread scrollback.
    pub thread_scroll: Scroll,
}

impl ConversationsState {
    /// The given roster, first entry selected, with the Transmit pane focused
    /// (body field) so the operator can type at once.
    pub(super) fn new(items: Vec<Conversation>) -> Self {
        Self {
            items,
            selected: 0,
            focus: Pane::Transmit,
...
impl App {
    ...
    pub fn upsert_peer(&mut self, kind: PeerKind, hash: String, name: Option<String>) {
        ...
        match kind {
            PeerKind::Delivery => {
                if let Some(conv) = self.convs.items.iter_mut().find(|c| c.peer == hash) {
                    if name.is_some() {
    ...
    pub fn start_conversation(&mut self, address: &str, alias: &str) -> bool {
        ...
        let idx = match self.convs.items.iter().position(|c| c.peer == key) {
            Some(i) => i,
            None => {
        ...
        if !alias.is_empty() {
            self.convs.items[idx].display_name = Some(alias.to_string());
            self.convs.items[idx].pinned = true;
        }
    ...
    pub fn select_next(&mut self) {
        if self.convs.selected + 1 < self.convs.items.len() {
            self.convs.selected += 1;
            self.mark_selected_read();
        }
    }
    ...
    pub(super) fn mark_selected_read(&mut self) {
        ...
        self.convs.thread_scroll.to_bottom();
        if let Some(conv) = self.selected_conv_mut() {
            conv.unread = 0;
    ...
    pub(super) fn cycle_selected_trust(&mut self) {
        let Some(conv) = self.convs.items.get_mut(self.convs.selected) else {
            return;
        };
    ...
    pub fn load_conversation(&mut self, mut loaded: Conversation) {
        if let Some(existing) = self.convs.items.iter_mut().find(|c| c.peer == loaded.peer) {
    ...
    pub fn transmit(&mut self) {
        let (body, title) = match self.convs.items.get(self.convs.selected) {
            Some(conv) => (
        ...
        let id = self.next_id();
        let conv = &mut self.convs.items[self.convs.selected];
    ...
    pub fn set_msg_status(&mut self, id: u64, status: MsgStatus) {
        let mut hit = None;
        for conv in &mut self.convs.items {
    ...
    pub fn set_location(&mut self, peer: &str, pos: GeoPos) {
        ...
        let changed = self.convs.items[idx].location != Some(pos);
        self.convs.items[idx].location = Some(pos);
        if changed {
            let label = self.convs.items[idx].label();
            self.push_log(format!(
