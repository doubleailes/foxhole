...
pub struct ShareZone {
    /// Highlighted row within [`MapState::zones`].
    pub selected: usize,
...
impl App {
    ...
    pub(super) fn open_share_zone(&mut self) {
        ...
        let Some(conv) = self.convs.list.get(self.convs.selected) else {
            return;
        };
    ...
    pub fn share_zone(&mut self, zone_idx: usize, peer: &str) {
        ...
        if let Some(conv) = self.convs.list.iter_mut().find(|c| c.peer == peer) {
...
mod tests {
    ...
    fn share_zone_enqueues_a_cot_event_and_echoes() {
        ...
        app.convs.list.push(Conversation::new("aa11"));
    ...
    fn revoke_shared_zone_sends_a_revocation_that_drops_on_the_receiver() {
        ...
        sender.convs.list.clear();
        sender.convs.list.push(Conversation::new("aa11"));
    ...
    fn share_picker_opens_only_with_a_peer_and_zone() {
        ...
        app.convs.list.push(Conversation::new("bb22"));
        app.convs.selected = 0;
        app.open_share_zone();
