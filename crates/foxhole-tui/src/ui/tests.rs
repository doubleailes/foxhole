...
fn map_app() -> crate::app::App {
    ...
    app.convs.items[0].display_name = Some("london".to_string());
    app.convs.items[0].location = Some(GeoPos::new(51.5, -0.12));
    app
}
...
fn world_map_renders_received_intel_and_review_modal() {
    ...
    app.convs.items[0].peer = "aa11bb22cc33".to_string();
    app.convs.items[0].trust = Trust::Trusted;
    app.apply_cot(
...
fn share_zone_modal_lists_local_zones_for_the_peer() {
    ...
    app.convs.items[0].display_name = Some("kilo".to_string());
    app.intel.share_zone = Some(ShareZone {
        selected: 0,
        peer: app.convs.items[0].peer.clone(),
