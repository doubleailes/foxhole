...
/// Network tool: peers and propagation nodes discovered via announces. Offline
/// (no `net` feature) these stay empty until live discovery feeds them.
fn render_network(frame: &mut Frame, app: &App, area: Rect) {
    let mut lines = vec!["This node (lxmf.delivery) — give this address to peers:".to_string()];
    match &app.local_address {
        Some(addr) => lines.push(format!("  {addr}")),
        None => lines.push("  (starting — address pending)".to_string()),
    }
    lines.push(String::new());
    lines.push("Delivery Peers".to_string());
    if app.conversations.is_empty() {
        lines.push("  (none discovered)".to_string());
    } else {
        for c in &app.conversations {
            lines.push(format!("  {}  {}", c.label(), c.peer));
        }
    }

    lines.push(String::new());
    lines.push("Propagation Nodes  [Up/Dn] select  [Enter] set active  [s] sync".to_string());
    if app.nodes.is_empty() {
        lines.push("  (none discovered)".to_string());
    } else {
        let active = app.config.propagation_node.as_deref();
        for (i, n) in app.nodes.iter().enumerate() {
            let marker = if i == app.node_selected { ">" } else { " " };
            let star = if active == Some(n.hash.as_str()) {
                "  [active]"
            } else {
                ""
            };
            let name = n.name.as_deref().unwrap_or("?");
            lines.push(format!("{marker} {name}  {}{star}", n.hash));
        }
    }
